...
struct BTree{
    ...
    int min(Node * root){
        ...
        int menor_esq = INT_MAX, menor_dir = INT_MAX;
        
        if(root -> left != nullptr){
            menor_esq = min(root -> left);
        }
        if(root -> right != nullptr){
            menor_dir = min(root -> right);
        }
        if(root -> value < menor_esq && root -> value < menor_dir){
            return root -> value;
        }
        else if(menor_esq < menor_dir && menor_esq < root ->value){
            return menor_esq;
        }
        else{
            return menor_dir;
